...
namespace LeiaLoft
{
    ...
    public abstract class AbstractLeiaStateTemplate : ILeiaState, IReleasable
    {
        ...
        public virtual void DrawImage(LeiaCamera camera, LeiaStateDecorators decorators)
        {
            ...
            bool isMultiCameraCompositing = false;
            Leia.Interlacer interlacer = LeiaDisplay.Instance.Interlacer;
            if (interlacer != null && camera.GetViewCount() == 2 && !isMultiCameraCompositing)
            {
                Texture[] inputViews = new Texture[]{camera.GetView(0).TargetTexture, camera.GetView(1).TargetTexture};
                interlacer.SetInputViews(inputViews);
                interlacer.SetOutput(interlacedAlbedoTexture);
                interlacer.Render();
            }
            else
            {
